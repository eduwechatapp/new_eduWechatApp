    ...
    wx.request({
      ...
      success(res){
        ...
        for(let i = 0;i<array.length;i++){
          htmlAry[i] = WpJson.html2json(array[i], 'returnData')
        ...
        //WxParse.wxParse('item', 'html', array, that);
      }
    })
  },
  toDetail:function(e){
    var that = this
    console.log(e)
    var id  = that.data.contentList[e.currentTarget.dataset.id].id
    wx.navigateTo({
      url: `../question_detail/question_detail?id=${id}&subject=${that.data.subject}`,
    })
  }
})